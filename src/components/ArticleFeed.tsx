...
import {
  fetchArticlesByTopics,
  getPopularWikipediaFeed,
...
export default function ArticleFeed({
  ...
  const topBar = (
    <div className="pointer-events-none fixed inset-x-0 top-4 z-30 px-4 md:top-6">
      <div className="mx-auto flex w-full items-start justify-between gap-3 md:max-w-md md:px-6">
        <button
          type="button"
    ...
    return (
      ...
      >
        ...
        <div className="max-w-xs">
          <div className="mx-auto size-10 animate-spin rounded-full border-2 border-white/15 border-t-white/90" />
  ...
  return (
    <section
      ref={feedRef}
      className="h-dvh w-full snap-y snap-mandatory overflow-y-auto bg-neutral-950 md:mx-auto md:max-w-md"
    ...
    >
      ...
      {!isLoading && !error && isLoadingMore ? (
        <div className="pointer-events-none sticky bottom-0 z-20 flex justify-center px-4 pb-6">
          <div className="rounded-full border border-white/10 bg-black/70 px-4 py-2 text-sm text-white/80 backdrop-blur">
            {copy.loadingMore}
          </div>
        </div>
      ) : null}
